...
;Variables
  ...
  !define PRODUCT_NAME "Electrum for Sugarchain"
  !define PRODUCT_WEB_SITE "https://github.com/sugarchain-project/electrum-sugar"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-sugar-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrum-sugar\electrum\gui\icons\electrum.ico"
...
Section
  ...
  File /r "dist\electrum-sugar\*.*"
  File "c:\electrum-sugar\electrum\gui\icons\electrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-sugar-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-sugar-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-sugar-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-sugar-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-sugar-${PRODUCT_VERSION}.exe" 0


  ;Links sugarchain: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\sugarchain" "" "URL:sugarchain Protocol"
  WriteRegStr HKCU "Software\Classes\sugarchain" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\sugarchain" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\sugarchain\shell\open\command" "" "$\"$INSTDIR\electrum-sugar-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\sugarchain"
